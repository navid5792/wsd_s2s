...
class LuongAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, last_hidden_pos, last_hidden_conv,encoder_outputs, encoder_outputs_pos,encoder_outputs_conv):
        ...
        attn_weights_1 = self.attn(rnn_output, encoder_outputs)
...
#        attn_weights = F.softmax(attn_weights.squeeze(1) * attn_weights_pos.squeeze(1)).unsqueeze(1) 
        attn_weights = F.softmax(self.W1*attn_weights_1.squeeze(1) + self.W2*attn_weights_pos.squeeze(1) + self.W3*attn_weights_conv.squeeze(1)).unsqueeze(1)
...
#        print("W1: ",self. W1.grad, "W2 = ", self.W2.grad)
        return output, hidden, hidden_pos,hidden_conv, attn_weights,attn_weights_1,attn_weights_conv,attn_weights_pos
...
def train(input_batches, input_lengths, target_batches, target_lengths, pos_batches, pos_lengths, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for t in range(max_target_length):
        decoder_output, decoder_hidden, decoder_hidden_pos,decoder_hidden_conv, decoder_attn,AT1,AT_conv,AT_pos = decoder(
    ...
    #print("After applying grad W1: ",decoder.W1.data,"grad :",decoder.W1.grad)
    #print("After applying grad W2: ",decoder.W2.data,"grad :",decoder.W1.grad)
    #decoder.W1 = decoder.W1 - decoder.W1.grad  # just testing subtraction
    #decoder.W2 = decoder.W3 - decoder.W2.grad
...
#    print("Grad of W2: ",decoder.W2.grad)
    return loss.data[0], ec, dc,decoder_attn,AT1,AT_conv,AT_pos
...
def evaluate(input_seq, pos_seq, max_length=MAX_LENGTH):
    ...
    decoder_AT1 = torch.zeros(max_length + 1, max_length + 1)
    decoder_AT_conv = torch.zeros(max_length + 1, max_length + 1)
    decoder_AT_pos = torch.zeros(max_length + 1, max_length + 1)
    
    
    # Run through decoder
    for di in range(max_length):
        decoder_output, decoder_hidden, decoder_hidden_pos,decoder_hidden_conv, decoder_attention,AT1,AT_conv,AT_pos = decoder(
            decoder_input, decoder_hidden, decoder_hidden_pos,decoder_hidden_conv, encoder_outputs, encoder_outputs_pos,encoder_outputs_conv
        )
        decoder_attentions[di,:decoder_attention.size(2)] += decoder_attention.squeeze(0).squeeze(0).cpu().data
        decoder_AT1[di,:AT1.size(2)] += AT1.squeeze(0).squeeze(0).cpu().data
        decoder_AT_conv[di,:AT_conv.size(2)] += AT_conv.squeeze(0).squeeze(0).cpu().data
        decoder_AT_pos[di,:AT_pos.size(2)] += AT_pos.squeeze(0).squeeze(0).cpu().data
    ...
    return decoded_words, decoder_attentions[:di+1, :len(encoder_outputs)],decoder_AT1[:di+1, :len(encoder_outputs)],decoder_AT_conv[:di+1, :len(encoder_outputs)],decoder_AT_pos[:di+1, :len(encoder_outputs)]
...
def show_attention(input_sentence, output_words, attentions):
    ...
    cax = ax.matshow(attentions.numpy(), cmap='PuRd')
    fig.colorbar(cax)
...
def evaluate_and_show_attention(input_sentence, pos, target_sentence=None):
    output_words, attentions,AT1,AT_conv,AT_pos = evaluate(input_sentence, pos)
    ...
    show_attention(input_sentence, output_words, AT1)
    show_attention(input_sentence, output_words, AT_conv)
    show_attention(input_sentence, output_words, AT_pos)
...
#    result_y = []
    ...
    for j in range(len(indexes)):
        ...
        if (len(input_sentence)) > 0:
            ...
            output_words, attentions,_,_,_ = evaluate(input_sentence, pos)
            if target_sentence is not None:
            ...
            for i in range(min(len(output_words), len(target_words))):
                ...
                if output_words[i] == target_words[i]:
                    right_num += 1
        result_x.append(dummy_x)
        result_y.append(dummy_y)
