...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        #print(start)
        #print(input_seqs.cpu().float().data)
...
def optimized_evaluate(test_pairs,check):
    ...
    if best_F1 < f1_score:
        torch.save(encoder.state_dict(),'best_encoder_conv')
        torch.save(decoder.state_dict(),'best_decoder_conv')
        best_F1 = f1_score
...
while epoch < n_epochs:
    ...
    if epoch % evaluate_every == 0:
        ...
        with open("result_conv.pkl","wb") as f:
...
encoder.load_state_dict(torch.load('best_encoder_conv'))
decoder.load_state_dict(torch.load('best_decoder_conv'))
