...
def optimized_evaluate(test_pairs,check):
    ...
    if best_F1 < f1_score:
        torch.save(encoder.state_dict(),'best_encoder_pos')
        torch.save(decoder.state_dict(),'best_decoder_pos')
        best_F1 = f1_score
...
while epoch < n_epochs:
    ...
    if epoch % evaluate_every == 0:
        ...
        with open("result_just_pos.pkl","wb") as f:
            pickle.dump([F1_score,ACC_seq,ACC_seq],f)
...
encoder.load_state_dict(torch.load('best_encoder_pos'))
decoder.load_state_dict(torch.load('best_decoder_pos'))
