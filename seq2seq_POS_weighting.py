...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        #print("embeddeddddddd")
        #print(embedded)
        #print("input_seeeeeeeeeeeeeeee")
        #print(input_seqs)
        #print("lengthsssssssssssssss")
        #print(input_lengths)
...
class LuongAttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, hidden_size, output_size, n_layers=1, dropout=0.1):
        ...
        self.gru_pos = nn.GRU(hidden_size, hidden_size, n_layers, dropout=dropout)
        self.concat = nn.Linear(hidden_size * 2, hidden_size)
        self.out = nn.Linear(hidden_size, output_size)
        self.W1 = nn.Parameter(torch.FloatTensor([0.8]).cuda(),requires_grad =True)
        self.W2 = nn.Parameter(torch.FloatTensor([0.2]).cuda(),requires_grad =True)
    ...
    def forward(self, input_seq, last_hidden, last_hidden_pos, encoder_outputs, encoder_outputs_pos):
        # Note: we run this one step at a time
        ...
        rnn_output_pos, hidden_pos = self.gru_pos(embedded, last_hidden_pos)
...
#        attn_weights = F.softmax(attn_weights.squeeze(1) * attn_weights_pos.squeeze(1)).unsqueeze(1) 
        attn_weights = F.softmax(self.W1*attn_weights.squeeze(1) + self.W2*attn_weights_pos.squeeze(1)).unsqueeze(1)
...
#        print("W1: ",self. W1.grad, "W2 = ", self.W2.grad)
        return output, hidden, hidden_pos, attn_weights
...
def train(input_batches, input_lengths, target_batches, target_lengths, pos_batches, pos_lengths, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    encoder_outputs_pos, encoder_hidden_pos = encoder(pos_batches, pos_lengths, None)
    ...
    for t in range(max_target_length):
        decoder_output, decoder_hidden, decoder_hidden_pos, decoder_attn = decoder(
            decoder_input, decoder_hidden, decoder_hidden_pos, encoder_outputs, encoder_outputs_pos
        )
...
#    print(decoder_attentions)
    ...
    for di in range(max_length):
        decoder_output, decoder_hidden, decoder_hidden_pos, decoder_attention = decoder(
            decoder_input, decoder_hidden, decoder_hidden_pos, encoder_outputs, encoder_outputs_pos
        )
...
#        i = indexes[j]
        ...
        import pickle
        with open('TRUE_PRED_POS_POS.pkl', "wb") as f:
    ...
    if best_F1 < f1_score:
        torch.save(encoder.state_dict(),'best_encoder_pos_weight')
        torch.save(decoder.state_dict(),'best_decoder_pos_weight')
        best_F1 = f1_score
...
#        F1_score.append(cal_culate_F1_score(test_pairs))
        ...
        with open("result_POS_weight.pkl","wb") as f:
...
encoder.load_state_dict(torch.load('best_encoder_pos_weight'))
decoder.load_state_dict(torch.load('best_decoder_pos_weight'))
